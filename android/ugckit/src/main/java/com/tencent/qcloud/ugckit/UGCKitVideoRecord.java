...
//        getTitleBar().setVisible(true, ITitleBarLayout.POSITION.RIGHT);
        ...
        getFilterPanel().setOnBeautyListener(new BeautyPanel.OnBeautyListener() {
    ...
    @Override
    public void setOnPickerClicked(RecordBottomLayout.OnPickerClicked onPickerClicked) {
        getRecordBottomLayout().setOnPickerClicked(onPickerClicked);
    }
    ...
    public void onShowFilterPanel() {
        getScrollFilterView().setPanelListener(new ScrollFilterView.OnPanelListener() {
            @Override
            public int getFilterProgress(int index) {
                return getFilterPanel().getFilterProgress(index);
            }

            @Override
            public Bitmap getFilterResource(int index) {
                return getFilterPanel().getFilterResource(index);
            }

            @Override
            public int getFilterSize() {
                return getFilterPanel().getFilterSize();
            }

            @Override
            public ItemInfo getFilterItemInfo(int index) {
                return getFilterPanel().getFilterItemInfo(index);
            }
        });
        // 隐藏底部工具栏
        ...
        if (getBeautyPanel().getmTxBeautyManager() == null) {
            ...
            getFilterPanel().setBeautyManager(txugcRecord.getBeautyManager());
            //设置默认美颜项
            VideoRecordSDK.getInstance().updateBeautyParam(new BeautyParams());
        }
        //恢复基础美颜属性
        getFilterPanel().restoreBeauty();
        mBeautyType = 0;
        if (mXMagic != null) {
            mXMagic.setAudioMute(true);
        }
    ...
    public void onShowBeautyPanel() {
        getScrollFilterView().setPanelListener(new ScrollFilterView.OnPanelListener() {
            @Override
            public int getFilterProgress(int index) {
                return getBeautyPanel().getFilterProgress(index);
            }

            @Override
            public Bitmap getFilterResource(int index) {
                return getBeautyPanel().getFilterResource(index);
            }

            @Override
            public int getFilterSize() {
                return getBeautyPanel().getFilterSize();
            }

            @Override
            public ItemInfo getFilterItemInfo(int index) {
                return getBeautyPanel().getFilterItemInfo(index);
            }
        });
        // 隐藏底部工具栏
        getRecordBottomLayout().setVisibility(View.GONE);
...
//    }
    ...
    @Override
    public boolean onCloseSuara() {
        // 隐藏底部工具栏
        getRecordBottomLayout().setVisibility(View.VISIBLE);
        // 隐藏右侧工具栏
        getRecordRightLayout().setVisibility(View.VISIBLE);
        getSoundEffectPannel().setVisibility(GONE);
        return true;
    }
...
//                        getRecordMusicPannel().setMusicName("");
    ...
    public void setConfig(UGCKitRecordConfig config) {
        ...
        // Record Speed
        getRecordRightLayout().setSpeed(config.mRecordSpeed);
    }
