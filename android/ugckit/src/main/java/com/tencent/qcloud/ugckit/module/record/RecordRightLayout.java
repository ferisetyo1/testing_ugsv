...
import static com.tencent.qcloud.ugckit.utils.ScreenUtils.dp2px;
...
public class RecordRightLayout extends RelativeLayout implements IRecordRightLayout,
        View.OnClickListener, AspectView.OnAspectListener, SpeedView.OnSpeedListener {
    ...
    private SpeedView mSpeedView;        // 屏比，目前有三种（1:1；3:4；9:16）
    ...
    private void initViews() {
        ...
        mSpeedView.setOnSpeedListener(this);

        mLayoutBeauty = (RelativeLayout) findViewById(R.id.layout_beauty);
        mImageBeauty = (ImageView) findViewById(R.id.iv_beauty);
//        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
//            mImageBeauty.setOutlineProvider(ViewOutlineProvider.BOUNDS);
//            mImageBeauty.setClipToOutline(true);
//            mImageBeauty.setElevation(2);
//        }
...
//            }
    ...
    public void setMusicIconEnable(boolean enable) {
        if (enable) {
            mLayoutMusic.setVisibility(VISIBLE);
//            mImageMusicMask.setVisibility(View.GONE);
        } else {
            mLayoutMusic.setVisibility(GONE);
//            mImageMusicMask.setVisibility(View.VISIBLE);
    ...
    public void setAspectIconEnable(boolean enable) {
        ...
        if (enable) {
            mAspectView.setVisibility(VISIBLE);
//            mAspectView.disableMask();
        } else {
//            mAspectView.enableMask();
            mAspectView.setVisibility(GONE);
        }
    ...
    public void setSpeed(int aspectRatio) {
        mSpeedView.setSpeed(aspectRatio);
    }

    @Override
    public void onSpeedSelect(int currentSpeed) {
        UGCKitRecordConfig.getInstance().mRecordSpeed = currentSpeed;
        VideoRecordSDK.getInstance().setRecordSpeed(currentSpeed);
    }
}
